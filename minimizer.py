...
def para_min(f,x, *args): 
	...
	num = 0.5*((x[2]**2-x[1]**2)*f(x[0],*args)+(x[0]**2-x[2]**2)*f(x[1],*args)+(x[1]**2-x[0]**2)*f(x[2],*args))
	denom = ((x[2]-x[1])*f(x[0],*args)+(x[0]-x[2])*f(x[1],*args)+(x[1]-x[0])*f(x[2],*args))
	x_min = num/denom
	return x_min
...
def minimise(f, start_points, epsilon, *args): 
	...
	conv = abs((points_hist[-1][0]-points_hist[-2][0])/points_hist[-2][0])
	i=1
	while conv>epsilon:
...
# plt.plot([elem[0] for elem in points_hist], [elem[1] for elem in points_hist], color='red', linestyle='', marker='.')
# plt.grid()
# plt.show()